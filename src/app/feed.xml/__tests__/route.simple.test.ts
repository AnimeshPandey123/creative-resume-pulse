...
describe('RSS Feed Route - Simple Tests', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (NextResponse as jest.Mock).mockReturnValue({
            headers: {},
        });
    });
    ...
    it('should generate valid RSS XML structure', async () => {
        ...
        const rssXml = (NextResponse as jest.Mock).mock.calls[0][0];
    ...
    it('should include all blog posts as RSS items', async () => {
        ...
        expect(rssXml).toContain('<link>https://animeshpandey.com/blog/test-blog-post-1</link>');
        expect(rssXml).toContain('<link>https://animeshpandey.com/blog/test-blog-post-2</link>');
    });
    ...
    it('should include feed image', async () => {
        ...
        expect(rssXml).toContain('<url>https://d1iukwsziul56d.cloudfront.net/drupal-local/s3fs-public/2025-08/IMG_3793%202.JPG</url>');
        expect(rssXml).toContain('<title><![CDATA[Animesh Pandey Blog]]></title>');
        expect(rssXml).toContain('<link>https://animeshpandey.com/blog</link>');
    ...
    it('should include build date and publication date', async () => {
        ...
        const buildDateMatch = rssXml.match(/<lastBuildDate>(.*?)<\/lastBuildDate>/);
    ...
    it('should handle errors gracefully', async () => {
        ...
        (NextResponse as jest.Mock).mockReturnValue(mockErrorResponse);

        await GET();

        expect(NextResponse).toHaveBeenCalledWith('Error generating RSS feed', { status: 500 });
