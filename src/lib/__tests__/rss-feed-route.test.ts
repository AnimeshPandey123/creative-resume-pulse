...
describe('RSS Feed Route Logic', () => {
  ...
  it('should validate RSS feed configuration', () => {
    ...
    // Note: SITE_CONFIG.author.email doesn't exist in the current config
    // We use a hardcoded email in the RSS feed instead
  ...
  it('should handle empty blog posts array', () => {
    ...
    const sortedEmptyPosts = [...emptyPosts].sort(
      (a, b) =>
        new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime()
    );
